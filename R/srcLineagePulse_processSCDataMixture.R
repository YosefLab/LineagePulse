...
processSCDataMixture <- function(
    ...
    # 2. dfAnnotation, vecConfoundersMu, boolFixedPopulations, vecNCentroidsPerPop
    if(!is.null(dfAnnotation)){
        ...
        if(!is.null(vecConfoundersMu)){
            if(!all(vecConfoundersMu %in% colnames(dfAnnotation))){
                stop(paste0("ERROR IN INPUT DATA CHECK: ",
                            "Not all confounders given in v given in dfAnnotation (columns)."))
            }
            if(any(is.null(dfAnnotation[,vecConfoundersMu]) | is.na(dfAnnotation[,vecConfoundersMu]))){
                stop(paste0("ERROR IN INPUT DATA CHECK: ",
                            "Supply batch assignments for all cells and all confounders given in vecConfoundersMu"))
            }
        }
        ...
        if(!is.null(vecConfoundersMu)){
            stop(paste0("ERROR IN INPUT DATA CHECK: ",
                        "vecConfoundersMu supplied but no dfAnnotation which carries batch structure."))
        }
        if(boolFixedPopulations){
            stop(paste0("ERROR IN INPUT DATA CHECK: ",
    ...
    if(!(strDispModel %in% c("constant", "MM"))){
        stop(paste0("ERROR IN INPUT DATA CHECK: ",
