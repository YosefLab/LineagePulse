...
fitMuDispGene <- function(
    ...
    lower <- -1e-10
    upper <- 1e10
    if (lsDispModelGlobal$strDispModel == "splines") {
        lower <- rep(-1e-10, vecTheta)
        lower[seq(1, lsDispModelGlobal$scaNSplines)] <- -10*log(10)

        upper <- rep(1e10, vecTheta)
        upper[seq(1, lsDispModelGlobal$scaNSplines)] <- 10*log(10)
    }

    fitDispMu <- tryCatch({
        unlist(optim(
            par=vecTheta,
            fn=evalLogLikMuDispGeneFit,
            vecCounts=vecCounts,
...
fitMuDispGeneImpulse <- function(
    ...
    if(boolUseNewInits){
        ...
        if(all(!is.na(vecLL))){
            ...
            # was successful choose best value
