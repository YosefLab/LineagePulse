...
#' Wrapper for log likelihood of negative binomial model
#' for a vector of counts.
#' 
#' NOT YET SUPPORTED.
...
evalLogLikMatrixNB <- function(
    ...
    if(boolConstModelOnMMLL &
       (is.null(matWeights) | 
        ...
        stop("boolConstModelOnMMLL usage wrong in evalLogLikMatrixNB().")
    }
        ...
        bplapply( seq_len(nrow(matCounts)), function(i){
            ...
            if(lsMuModel$lsMuModelGlobal$strMuModel=="MM"){
                ...
                if(boolConstModelOnMMLL) {
                    ...
                    scaLL <- evalLogLikNB(
                        vecCounts=vecCounts,
                        vecMu=vecMuParam * lsMuModel$lsMuModelGlobal$vecNormConst,
