#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
#++++++++     Co-Fit mean and dispersion parameters of NB model    +++++++++#
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
# File divided into:
# ++++ Model cost functions -evalLogLikNB (not in this file)
# +++ Fitting cost functions: return loglik of proposed parameters
# +++ - evalLogLikContinuousNB 
# ++ Optim wrappers for single fits
# ++ - fitContinuousNB wrapper for single initialisation on one gene
# ++ - fitImpulseNB wrapper for multiple initialisation of impulse model
# + Overall model fitting wrapper: 
# + Top level auxillary function called by fitNB.
...
#' Cost function negative binomial model for mean and
...
#' of mean and dispersion paramater model on single gen.
#' 
...
evalLogLikContinuousNB <- function(
    vecTheta,
    vecCounts,
    lsMuModelGlobal,
    lsDispModelGlobal,
...
#' Compiled function: evalLogLikContinuousNB
#' 
#' Pre-compile heavily used functions.
#' Refer to \link{evalLogLikContinuousNB}.
#' 
#' @seealso \link{evalLogLikContinuousNB}
#' 
#' @param vecTheta (numeric vector dispersion (1) and impulse parameters (6)) 
#' Dispersion and mean parameter estimates.
#' @param vecCounts (count vector number of cells)
...
#'
#' @author David Sebastian Fischer
evalLogLikContinuousNB_comp <- compiler::cmpfun(evalLogLikContinuousNB)
...
#' cost function.
#' 
#' @seealso Called once for each gene by \code{fitNBMuDisp}
#' or within wrapper \code{fitNBImpulse} 
#' once for each initalisation of each gene.
#' Calls fitting likelihood functions: 
#' \code{evalLogLikContinuousNB_comp}.
#' 
#' @param vecCounts (count vector number of cells)
#'Observed read counts, not observed are NA.
...
fitContinuousNB <- function(
    ...
    fitDispMu <- tryCatch({
        unlist(optim(
            par=vecTheta,			
            fn=evalLogLikContinuousNB_comp, 
            vecCounts=vecCounts,
        ...
        message(paste0("ERROR: Fitting impulse model: fitContinuousNB()."))
        message(strErrorMsg)
...
#' this wrapper sits ontop of fitContinuousNB() in the fitting hierarchy
#' and wraps multiple initialisations at the level of one gene.
#' 
#' @seealso Called by mean-dispersion co-estimation wrapper 
#' \code{fitNBMuDisp}.
#' Calls optimisation wrapper \code{fitContinuousNB} 
...
fitMMNB <- function(
    ...
    stop("LineagePulse ERROR: fitMMNB() not yet supported.",
         " Contact developer.")
    return(NULL)
}
...
#' models.
#' 
#' @seealso Called by \code{fitNB}. Calls fitting wrappers:
#' \code{fitImpulseNB},
#' \code{fitContinuousNB}.
#' 
