...
* counts the count matrix (genes x cells) which MUST NOT be normalised in any way. A valid input option is expected counts from an aligner. Note that TPM or depth normalised expected counts are NOT count data. The statistical framework of LineagePulse rests on the assumption that matCounts contain count data (similar to DESeq2 and other count-based differential expression methods). The count matrix can also be supplied as the path to a .mtx file (sparse matrix format file) or as a SummarizedExperiment or SingleCellExperiment object.
* dfAnnotation data frame that contains a column named "continuous" if a continuous model is fit (e.g. to time or pseudotime coordinates) and which contains columns that describe the batch structure (if any). If LineagePulse is run across discrete groups of cells (e.g. clusters or conditions), strMuModel can be chosen to be "groups" and dfAnnotation must contain a column named "groups" that contains the assignemnt of cells to these groups as strings. The rownames of dfAnnotation must be equal to the column names of matCounts as both correspond to cells. Note that if counts is a SummarizedExperiment or SingleCellExperiment object, the annotation data frame is taken to be colData(counts) if dfAnnotation is NULL.
...
print(paste0("Minimum fitted mean parameter: ", round(min(matMeanParamFit),1) ))
print(paste0("Mean fitted mean parameter: ", round(mean(matMeanParamFit),1) ))
```
...
print(paste0("LFC minimum to maximum expression value of model fit: ", 
