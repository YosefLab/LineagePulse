...
* dfAnnotation data frame that contains a column names "pseudotime" if a continuous model is fit and which contains columns that describe the batch structure (if any). The rownames of dfAnnotation must be equal to the column names of matCounts as both correspond to cells. Note that if counts is a SummarizedExperiment or SingleCellExperiment object, the annotation data frame is taken to be colData(counts) if dfAnnotation is NULL.
...
Of interest may be for example the fold change from the first to the last  beginning
...
matMeanParamFit <- getFitsMean(lsMuModel = lsMuModelH1(objLP),
                               vecGeneIDs = objLP$dfResults[which.min(objLP$dfResults$p),]$gene)
print(paste0("Minimum fitted mean parameter: ", min(matMeanParamFit)))
print(paste0("Mean fitted mean parameter: ", mean(matMeanParamFit)))
```
